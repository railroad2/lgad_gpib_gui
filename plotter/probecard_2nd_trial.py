...
sensor_name = 'W9a'
probecard = 'C:/LGAD_test/I-V_test/'+sensor_name+'/IV_SMU+PAU_'+sensor_name+'_probecard_test_'+date+'_'+'pad2_v0.txt'
plotter.add_input_data(probecard, label="Probecard", y_index=2, color=colors[0], linewidth=2, draw_half=True, y_scale=1e3, flip_y=True)
...
plotter.add_input_data(probecard_many_ground, label="Probecard (more ground)", y_index=2, color=colors[1], linewidth=2, draw_half=True, y_scale=1e3, flip_y=True)

date = '2024-08-20'
probestation = 'C:/LGAD_test/I-V_test/'+sensor_name+'/IV_SMU+PAU_'+sensor_name+'_two_contact_rightmost_'+date+'_'+'pad4_v0.txt'
plotter.add_input_data(probestation, label="probe station", y_index=2, color=colors[2], linewidth=2, draw_half=True, y_scale=1e3, flip_y=True)
...
plotter.set_experiment_label(location=(0, 0), label='W9a 16x16(diced)', fontsize=20)
