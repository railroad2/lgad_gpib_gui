...
input_data_pad3 = 'C:/LGAD_test/I-V_test/'+date+'_'+sensor_name+'/IV_SMU+PAU_'+sensor_name+'_'+date+'_'+initial_voltage+'_'+final_voltage+'_pad0_2_3.txt'
...
plotter.add_input_data(input_data_pad1, label="First sweep", color=colors[0], linewidth=2, draw_half=True)
plotter.add_input_data(input_data_pad1, label='First return sweep', color=colors[0], linewidth=2, linestyle='--', draw_second_half=True)
plotter.add_input_data(input_data_pad2, label="Second sweep", color=colors[1], linewidth=2, draw_half=True)
plotter.add_input_data(input_data_pad3, label="Third sweep", color=colors[2], linewidth=2, draw_half=True)
...
plotter.set_experiment_label(location=(0, 0), label='W13 with probecard', fontsize=20)
