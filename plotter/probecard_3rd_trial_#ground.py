...
plotter.add_input_data(probecard, label= coln + ' ' + rown + ' 256 ground', y_index=2, color=colors[0], linewidth=2, draw_half=True, y_scale=1e3, flip_y=True)
...
plotter.add_input_data(probecard, label= coln + ' ' + rown + ' 192 ground', y_index=2, color=colors[1], linewidth=2, draw_half=True, y_scale=1e3, flip_y=True)
...
plotter.add_input_data(probecard, label= coln + ' ' + rown + ' 96 ground', y_index=2, color=colors[2], linewidth=2, draw_half=True, y_scale=1e3, flip_y=True)
...
plotter.save_fig(out_name=sensor_name+'_#ground_iv')
